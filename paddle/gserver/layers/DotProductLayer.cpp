...
#include "DotProductLayer.h"
#include "paddle/utils/Logging.h"
#include "paddle/utils/Stat.h"

namespace paddle {

REGISTER_LAYER(dot_product, DotProductLayer);

bool DotProductLayer::init(const LayerMap& layerMap,
...
void DotProductLayer::forward(PassType passType) {
  ...
  /* activation */ {
    ...
    outV->dotProduct(*prevOut1, *prevOut2, config_.cos_scale());
  }
}

void DotProductLayer::backward(const UpdateCallback& callback) {
  /* activation */ {
    // REGISTER_TIMER_INFO("CosBpAtvTimer", getName().c_str());
    MatrixPtr outG = this->getOutputGrad();

    outG->dotProductDerivative(*this->getOutputValue(), *getInputValue(0),
